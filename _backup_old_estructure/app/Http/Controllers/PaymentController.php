...
class PaymentController extends Controller 
{
    ...
    public function openpayWebhookConfirm(Request $request)
    {
        ...
        if ($transaction['status'] == 'completed') {
            ...
            if (!$transactionId) return response()->json(['success' => 'success'], 200);

            $lockKey = 'webhook_openpay_' . $transactionId;
            $lock = \Illuminate\Support\Facades\Cache::lock($lockKey, 30);
            
            if (!$lock->get()) {
                Log::warning('Webhook ya está siendo procesado por otro hilo', ['transaction_id' => $transactionId]);
                return response()->json(['success' => 'success'], 200);
            }
            ...
            // ✅ NOTIFICAR A N8N — Caso: Transacción NO completada
